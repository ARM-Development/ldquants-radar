...
def help_message():
    ...
    """
    print("\n")
    print("Syntax: ldquantsRADAR.py <-h> <-p> LDQUANTS_M1_file"
          + "LDQUANTS_S1_file\n")
    print("  INPUT:  ")
...
def pull_allscans(connection, site, ndatetime, outdir=None):
    ...
    if str(ndatetime.year) not in connection.get_avail_years():
        ...
        if (f'{ndatetime.month:02}' not in
            ...
            print('ERROR: Month Not Found For Radar Site\n')
            sys.exit(1)
        else:
            if (f'{ndatetime.day:02}' not in
                    connection.get_avail_days(ndatetime.year,
                                              ndatetime.month)):
                print('ERROR: Day Not Found For Radar Site\n')
                sys.exit(1)
...
for loc_file in downloads.iter_success():
    ...
    if loc_file.filepath[-3:] != 'MDM':
        ...
        (rhi_data_m1, rhix_m1,
            rhiy_m1, rhiz_m1) = (pyart.graph.RadarDisplay(radar)
        ...
        (rhi_data_s1, rhix_gci,
            rhiy_s1, rhiz_s1) = (pyart.graph.RadarDisplay(radar)
                                 ._get_azimuth_rhi_data_x_y_z(
        ...
        # find gates for that location
        # Note: x,y,z gates are in km
        (rain_data_m1, rainx_m1,
            rainy_m1, rainz_m1) = (pyart.graph.RadarDisplay(radar)
                                   ._get_azimuth_rhi_data_x_y_z(
                                   'est_rainfall_rate',
                                   azim_M1, edges=True,
                                   mask_tuple=None,
        ...
        (rain_data_s1, rainx_s1,
            rainy_s1, rainz_s1) = (pyart.graph.RadarDisplay(radar)
                                   ._get_azimuth_rhi_data_x_y_z(
                                   'est_rainfall_rate',
                                   azim_S1,
                                   edges=True, mask_tuple=None,
        ...
        begin_time = (radar.time['units'].split(' ')[2].
                      split('T')[-1][0: -1])
        ...
        mask_x, mask_y = np.where(rhi_data_m1 != np.ma.masked)
        mask_xs, mask_ys = np.where(rhi_data_s1 != np.ma.masked)

        # fill the ndata dictionary for further analysis
        if time_sync_M1.size > 0:
            ...
            if np.ma.sum(nc_M.variables['rain_rate'][time_sync_M1] is not
                         np.ma.masked):
                tempx = np.ma.average(nc_M.variables['rain_rate']
                                      [time_sync_M1])
                ndata_m1['LD_rain'].append(np.ma.around(tempx, 2))
            else:
                ndata_m1['LD_rain'].append(-999)

            if (np.ma.average(nc_M.variables['reflectivity_factor_sband20c']
                              [time_sync_M1]) is not np.ma.masked):
                tempy = np.ma.average((nc_M.variables[
                                       'reflectivity_factor_sband20c']
                                       [time_sync_M1]))
                ndata_m1['LD_Z'].append(np.ma.around(tempy, 2))
            else:
                ndata_m1['LD_Z'].append(-999)

            if time_sync_V1.size > 0:
                if (np.ma.sum(nc_V.variables['rain_rate'][time_sync_V1])
                        is not np.ma.masked):
                    tempz = np.ma.average(nc_V.variables['rain_rate']
                                          [time_sync_V1])
                    ndata_m1['VD_rain'].append(np.ma.around(tempz, 2))
                else:
                    ndata_m1['VD_rain'].append(-999)

                if (np.ma.average(nc_V.variables[
                        'reflectivity_factor_sband20c'][time_sync_V1])
                        is not np.ma.masked):
                    tempa = np.ma.average((nc_V.variables[
                                          'reflectivity_factor_sband20c']
                                          [time_sync_V1]))
                    ndata_m1['VD_Z'].append(np.ma.around(tempa, 2))
                else:
                    ndata_m1['VD_Z'].append(-999)

        if time_sync_S1.size > 0:
            ndata_s1['date'].append(radar.time['units'].split(' ')[2][0: 10])
            ndata_s1['time'].append(sfm_begin_time)
            ndata_s1['xGate'].append(rhix_gci[mask_xs, mask_ys][0])
            ndata_s1['yGate'].append(rhiy_s1[mask_xs, mask_ys][0])
            ndata_s1['zGate'].append(rhiz_s1[mask_xs, mask_ys][0])
            ndata_s1['tarDis'].append(dis_S1[0]/1000.)
            ndata_s1['tarAzi'].append(azim_S1[0])
            ndata_s1['rhi_data'].append(rhi_data_s1[mask_xs, mask_ys][0])
            ndata_s1['rain_data'].append(rain_data_s1[mask_xs, mask_ys][0])
            if (np.ma.sum(nc_S.variables['rain_rate'][time_sync_S1]) is not
                    np.ma.masked):
                tempb = np.ma.average(nc_S.variables['rain_rate']
                                      [time_sync_S1])
                ndata_s1['LD_rain'].append(np.ma.around(tempb, 2))
            else:
                ndata_s1['LD_rain'].append(-999)

            if (np.ma.average(nc_S.variables['reflectivity_factor_sband20c']
                              [time_sync_S1]) is not np.ma.masked):
                tempc = np.ma.average((nc_S.variables[
                                       'reflectivity_factor_sband20c']
                                       [time_sync_S1]))
                ndata_s1['LD_Z'].append(np.ma.around(tempc, 2))
            else:
                ndata_s1['LD_Z'].append(-999)
